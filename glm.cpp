...
preservation of edges, welding redundant vertices_ & texture
...
* statistics of the model (such as #vertices_, #normals, etc)
*
* model - properly initialized GLMmodel structure
...
{
	GLuint numvertices;        /* number of vertices_ in model */
...
glmSecondPass(GLMmodel* model, FILE* file)
{
	...
	GLuint numtexcoords;       /* number of texcoords in model */
	GLuint numtriangles;       /* number of triangles in model */
	GLfloat* vertices_;         /* array of vertices_  */
	...
	while (fscanf(file, "%s", buf) != EOF) {
		...
		case 'v':               /* v, vn, vt */
			...
			case '\0':          /* vertex */
				fscanf(file, "%f %f %f",
					&vertices_[3 * numvertices + 0],
					&vertices_[3 * numvertices + 1],
					&vertices_[3 * numvertices + 2]);
				// 		printf("vertices_%d %f %f %f\n",numvertices,vertices_[3 *numvertices + 0],vertices_[3 * numvertices+ 1],vertices_[3 * numvertices + 2]);
	...
	//
	//      printf("vertices_%d %f %f %f\n",i,model->vertices_[3 * i + 0],model->vertices_[3 * i + 1],model->vertices_[3 * i + 2]);
	//    }
#if 0
...
glmUnitize(GLMmodel* model)
{
	...
	assert(model);
	assert(model->vertices_);

	/* get the max/mins */
	maxx = minx = model->vertices_[3 + 0];
	maxy = miny = model->vertices_[3 + 1];
	maxz = minz = model->vertices_[3 + 2];
	for (i = 1; i <= model->numvertices; i++) {
		if (maxx < model->vertices_[3 * i + 0])
			maxx = model->vertices_[3 * i + 0];
		if (minx > model->vertices_[3 * i + 0])
			minx = model->vertices_[3 * i + 0];

		if (maxy < model->vertices_[3 * i + 1])
			maxy = model->vertices_[3 * i + 1];
		if (miny > model->vertices_[3 * i + 1])
			miny = model->vertices_[3 * i + 1];

		if (maxz < model->vertices_[3 * i + 2])
			maxz = model->vertices_[3 * i + 2];
		if (minz > model->vertices_[3 * i + 2])
			minz = model->vertices_[3 * i + 2];
	}
	...
	for (i = 1; i <= model->numvertices; i++) {
		model->vertices_[3 * i + 0] -= cx;
		model->vertices_[3 * i + 1] -= cy;
		model->vertices_[3 * i + 2] -= cz;
		/* model->vertices_[3 * i + 0] *= scale;
		model->vertices_[3 * i + 1] *= scale;
		model->vertices_[3 * i + 2] *= scale;*/
	}
...
glmFacetNormals(GLMmodel* model)
{
	...
	for (i = 0; i < model->numtriangles; i++) {
		...
		u[0] = model->vertices_[3 * T(i).vindices[1] + 0] -
			model->vertices_[3 * T(i).vindices[0] + 0];
		u[1] = model->vertices_[3 * T(i).vindices[1] + 1] -
			model->vertices_[3 * T(i).vindices[0] + 1];
		u[2] = model->vertices_[3 * T(i).vindices[1] + 2] -
			model->vertices_[3 * T(i).vindices[0] + 2];

		v[0] = model->vertices_[3 * T(i).vindices[2] + 0] -
			model->vertices_[3 * T(i).vindices[0] + 0];
		v[1] = model->vertices_[3 * T(i).vindices[2] + 1] -
			model->vertices_[3 * T(i).vindices[0] + 1];
		v[2] = model->vertices_[3 * T(i).vindices[2] + 2] -
			model->vertices_[3 * T(i).vindices[0] + 2];
...
* linearly mapping the vertices_ onto a square.
*
...
glmLinearTexture(GLMmodel* model)
{
	...
	/* do the calculations */
	for (i = 1; i <= model->numvertices; i++) {
		x = model->vertices_[3 * i + 0] * scalefactor;
		y = model->vertices_[3 * i + 2] * scalefactor;
...
glmReadOBJ(const char* filename)
{
	...
	of vertices_, normals, texcoords & triangles */
	glmFirstPass(model, file);
	//     model->printV();

	/* allocate memory */
	model->vertices_ = (GLfloat*)malloc(sizeof(GLfloat) *
...
*             GLM_NONE     -  render with only vertices_
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
	...
	/* spit out the vertices_ */
	fprintf(file, "\n");
	fprintf(file, "# %d vertices\n", model->numvertices);
	for (i = 1; i <= model->numvertices; i++) {
		fprintf(file, "v %f %f %f\n",
			model->vertices_[3 * i + 0],
			model->vertices_[3 * i + 1],
			model->vertices_[3 * i + 2]);
	}
...
glmDraw(GLMmodel* model, GLuint mode)
{
	...
	while (group) {
		...
		for (i = 0; i < group->numtriangles; i++) {
			...
			glVertex3fv(&model->vertices_[3 * triangle->vindices[0]]);

			if (mode & GLM_SMOOTH)
				glNormal3fv(&model->normals[3 * triangle->nindices[1]]);
			if (mode & GLM_TEXTURE)
				glTexCoord2fv(&model->texcoords[2 * triangle->tindices[1]]);
			glVertex3fv(&model->vertices_[3 * triangle->vindices[1]]);

			if (mode & GLM_SMOOTH)
				glNormal3fv(&model->normals[3 * triangle->nindices[2]]);
			if (mode & GLM_TEXTURE)
				glTexCoord2fv(&model->texcoords[2 * triangle->tindices[2]]);
			glVertex3fv(&model->vertices_[3 * triangle->vindices[2]]);
...
*
* model   - initialized GLMmodel structure
* epsilon     - maximum difference between vertices_
...
glmWeld(GLMmodel* model, GLfloat epsilon)
{
	...
	/* free space for old vertices_ */
	free(vectors);

	/* allocate space for the new vertices_ */
	model->numvertices = numvectors;
	model->vertices_ = (GLfloat*)malloc(sizeof(GLfloat) *
		3 * (model->numvertices + 1));

	/* copy the optimized vertices_ into the actual vertex list */
	for (i = 1; i <= model->numvertices; i++) {
		model->vertices_[3 * i + 0] = copies[3 * i + 0];
		model->vertices_[3 * i + 1] = copies[3 * i + 1];
		model->vertices_[3 * i + 2] = copies[3 * i + 2];
	}
